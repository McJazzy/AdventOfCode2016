...
struct state { std::string path;  pos p;};
...
std::string process<17>(std::istream & is, bool part2) {
	...
	state orig_s;
	orig_s.path = ""; orig_s.p = orig;
	std::array<state, 4> dirs; 
	dirs[0] =  { "U", {0, 1}};
	dirs[1] = { "D", { 0, -1 }};
	dirs[2] = { "L",{ -1, 0 }};
	dirs[3] = { "R", { 1, 0} };
	//std::vector<state> dirs; dirs.push_back({ "U",{{ 0,1 }} }); dirs.push_back({ "D",{{ 0,-1 }} }); dirs.push_back({ "L",{{ -1,0 }} }); dirs.push_back({ "R",{{ 1,0 }} });
...
template <> 
void solve<17>() {
	std::cout << "advent17: " << process<17>(std::stringstream("hhhxzeay"), false) << std::endl;
}
...
void test<17>() {
	...
	assert(process<17>(std::stringstream("ulqzkmiv"), false) == "DRURDRUDDLLDLUURRDULRLDUUDDDRR");
}